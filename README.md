...
- **顯示顏色**：
  當 `unit` 增加時，對應的 LED 單元會變為香蕉黃（RGB：255, 255, 71）。當 `unit` 超過設定的最大 LED 數量（`NUM_LEDS_TOTAL`），會重置並熄燈。
...
顏色會通過 RGB 值來設置，在這個範例中使用的是「香蕉黃」(RGB: 255, 255, 71)，您可以根據需要修改此值來顯示不同顏色。
...
- `NUM_LEDS_TOTAL`：總 LED 燈條的數量，可以根據您的實際燈條調整。
- `LED_PIN`：設定控制 LED 的引腳，根據您連接的引腳進行修改。
- `BUTTON_PINS`：設定按鈕引腳，根據您使用的引腳進行修改。
